        ...
        <div class="col-12 col-sm-6 col-md-3 col-lg-2 mt-2">
          <div class="input-field-container">
            ...
            <select name="role" class="styled-input">
              ...
              <option value="nannies" <?= $employee['role'] == 'nannies' ? 'selected' : ''; ?>>Nannies</option>
...
<?php if (!empty($employee['police_verification_document'])): ?>
    <a href="../uploads/<?= htmlspecialchars($employee['police_verification_document']); ?>"
       target="_blank"
       class="text-primary file-link text-decoration-none">
        View Document
    </a>
<?php endif; ?>
...
<div class="col-12 col-sm-6 col-md-3 col-lg-3 mt-3" id="vendorFields" style="<?= $employee['reference'] === 'vendors' ? '' : 'display: none;' ?>">
    <div class="input-field-container">
        <div class="d-flex align-items-center">
            ...
            <select name="vendor_name" id="vendor_name" class="styled-input form-control">
    ...
    if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
            $selected = ($employee['vendor_name'] == $row['vendor_name']) ? 'selected' : '';
            echo "<option 
    ...
    <?php if (!empty($document['file_path'])): ?>
        <span class="text-muted ms-2">
            <a href="<?= htmlspecialchars($document['file_path']); ?>" target="_blank" class="text-decoration-none">
                View Document
            </a>
        </span>
    <?php endif; ?>
                ...
                <div class="input-field-container">
        <label class="input-label" id="aadharLabel">
...
<?php if (!empty($employee['adhar_upload_doc'])): ?>
    <div class="mt-2">
        <a href="../uploads/<?= htmlspecialchars($employee['adhar_upload_doc']); ?>" 
           target="_blank" 
...
<?php endif; ?>
  ...
  <div class="col-md-12 text-center">
    ...
    const vendorId = selectedOption.dataset.id;

    if (vendorId) {
        fetch(`fetch_vendor_edit_data.php?vendor_id=${vendorId}`)
            .then(response => response.json())
            .then(data => {
                if (data && data.length > 0) {
                    const vendor = data[0];
                    document.getElementById('vendor_contact').value = vendor.phone_number || '';
                    document.getElementById('beneficiary_name').value = vendor.beneficiary_name || '';
                    document.getElementById('bank_name').value = vendor.bank_name || '';
                    document.getElementById('branch').value = vendor.branch || '';
                    document.getElementById('bank_account_no').value = vendor.account_number || '';
                    document.getElementById('ifsc_code').value = vendor.ifsc || '';
                } else {
                    console.error('No vendor data found for the selected ID.');
                }
            })
            .catch(error => console.error('Error fetching vendor data:', error));
    } else {
        console.error('Vendor ID is not selected.');
    }
});
...
document.getElementById('vendor_name').addEventListener('change', function () {
    ...
    // Fetch related data from the selected option's data attributes
const beneficiaryName = selectedOption.getAttribute('data-contact') || ''; // Assuming data-contact is for beneficiary name
...
// Populate the fields
document.getElementById('beneficiary_name').value = beneficiaryName; // Corrected to use beneficiaryName
document.getElementById('bank_name').value = bankName;
document.getElementById('branch').value = branch;
document.getElementById('bank_account_no').value = accountNumber;
