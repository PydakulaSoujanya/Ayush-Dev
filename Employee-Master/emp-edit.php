...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id'])) {
    ...
    // Prepare the SQL query for employee details
    ...
    if ($stmt->execute()) {
        ...
        // Fetch vendor information if reference is 'vendors'
        if ($employee['reference'] === 'vendors') {
            $vendor_name = $employee['vendor_name'];
            $vendor_contact = $employee['vendor_contact'];
        }
    } else {
    ...
    // Fetch addresses from emp_addresses table
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
    ...
    $documentPathForDB = $employee['police_verification_document']; // Retain the old value by default
...
if (!empty($_FILES['police_verification_document']['name'])) {
    ...
    if (move_uploaded_file($_FILES['police_verification_document']['tmp_name'], $documentPath)) {
        ...
        if (!empty($employee['police_verification_document'])) {
            $oldFilePath = $employee['police_verification_document'];
            if (file_exists($oldFilePath)) {
                unlink($oldFilePath);
            }
        }

        // Save new file path for DB
        $documentPathForDB = "../uploads/" . $newDocumentName;
    } else {
    ...
    // }
    $emp_id = $employee['id']; // Get employee ID
...
$stmt->bind_param(
  ...
  $aadhar,
  $police_verification,
  $documentPathForDB, // Use the updated file path
  $daily_rate,
  $status,
  $termination_date,
  $bank_name,
  ...
  $employee_id
    ...
    if ($stmt->execute()) {
        ...
        $sql = "INSERT INTO emp_addresses (emp_id, address_line1, address_line2, city, state, pincode, landmark) 
                VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        ...
        foreach ($address_line1 as $index => $line1) {
            ...
            $pincode_val = $pincode[$index];
            $landmark_val = $landmark[$index];
  ...
  <div class="input-field-container">
    <label class="input-label">
      ...
      <?php if (!empty($employee['adhar_upload_doc'])): ?>
        <!-- Display the document name inside the label if it exists -->
...
<div class="col-md-3" id="vendorFields" style="display: none;">
  ...
  <div class="d-flex align-items-center">
        ...
        <select name="vendor_name" id="vendor_name" class="styled-input form-control me-2" value="<?= htmlspecialchars($employee['vendor_name']); ?>" />
...
<div class="col-md-3" id="vendorContactField" style="display: none;">
    <div class="input-field-container">
      ...
      <input type="text" id="vendor_contact" name="vendor_contact" class="styled-input" placeholder="Enter Vendor Contact Number" pattern="[0-9]{10}"  value="<?= htmlspecialchars($employee['vendor_contact']); ?>" />
    </div>
  </div>
  ...
  <div class="col-md-3">
    ...
    <div class="input-field-container">
        ...
        <div id="document-card-container" class="mt-3">
            <?php
            // Assuming $emp_id is the employee ID for the current employee
            // Query to get the employee's documents from the emp_documents table
